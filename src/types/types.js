

export const types =  {

    usuairiosCargados: '[Chat] Usuarios cargados', 
    activarChat: '[Chat] Activar Chat',
    nuevoMensaje: '[Chat] Nuevo Mensaje',
    cargarMensajes: '[Chat] Cargar Mensajes',
    // usuairiosCargados: '[Chat] Usuarios cargados'
    
    // Nuevos tipos para grupos
    gruposCargados: '[Chat] Grupos cargados',
    agregarGrupo: '[Chat] Agregar Grupo',
    cargarGrupo: '[Chat] Cargar grupo',
    activarGrupo: '[Chat] Activar Grupo',
    actualizarCanvas: '[Chat] Actualizar Canvas',
    usuarioUnidoGrupo: '[Chat] Usuario Unido Grupo',
    usuarioSalidoGrupo: '[Chat] Usuario Salido Grupo'

}
